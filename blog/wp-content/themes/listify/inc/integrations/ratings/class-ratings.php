...
class Listify_Ratings extends listify_Integration {
	...
	public function comment_form_top() {
		...
		for ( $i = 5; $i > 0; $i-- ) {
			$output[] = sprintf(
				'<a class="star-rating" href="" data-rating="%d"></a>',
				$i
			);
		}
		...
		echo '<span class="star-rating-stars">';
		echo implode( '', $output );
		echo '</span>';
		echo '</p>';
	}
	...
	public function comment_post( $comment_id ) {
		...
		$rating = isset( $_POST[ 'comment_rating' ] ) ? absint( $_POST[ 'comment_rating' ] ) : null;
	...
	?>
		<div itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating" class="job_listing-rating-wrapper" title="<?php printf( '%d Reviews', $rating->count() ); ?>">
			...
			<span class="job_listing-rating-count">
				<?php echo _n( sprintf( '<span itemprop="reviewCount">%d</span> Review', $rating->count() ), sprintf( '<span
				itemprop="reviewCount">%d</span> Reviews', $rating->count() ), $rating->count(), 'listify' ); ?>
			</span>
		</div>
	<?php
	}
