...
function listify_get_cover_from_group( $args ) {
	...
	if ( ! $args[ 'images' ] && $args[ 'object_ids' ] ) {
		...
		$image = wp_get_attachment_image_src( current( $args[ 'images' ] ), 'full' );
	}
...
$integrations = apply_filters( 'listify_integrations', array(
	...
	'wp-job-manager-wc-paid-listings' => class_exists( 'WP_Job_Manager_WCPL' ),
