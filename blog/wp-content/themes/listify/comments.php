...
<div id="comments" class="comments-area widget widget-job_listing">
	...
	<?php if ( have_comments() ) : ?>
		...
		<ol class="commentlist">
			<?php
				...
				), get_comments( array( 'post_id' => get_post()->ID ) ) );
			?>
		</ol><!-- .commentlist -->

		<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through ?>
		<nav id="comment-nav-below" class="comment-navigation" role="navigation">
			<h1 class="screen-reader-text"><?php _e( 'Comment navigation', 'listify' ); ?></h1>
	...
	<?php
		...
		$label_submit = is_singular( 'job_listing' ) ? __( 'Publish Review', 'listify' ) : __( 'Publish Reply', 'listify' );
		...
		comment_form( array(
			'title_reply' => '<span class="ion-' . $comments_widget_icon . '"></span>' . $comments_widget_title,
			'title_reply_to' => '<span class="ion-' . $comments_widget_icon . '"></span>' . $comments_widget_title,
